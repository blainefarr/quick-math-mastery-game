      ...
      <Collapsible open={isOpen} onOpenChange={handleToggle} className="w-full">
        ...
        <CollapsibleContent>
          <div className="rounded-lg border bg-muted/5 p-4 space-y-4 mt-2">
            <FocusNumberSection enabled={useFocusNumber} value={focusNumberValue} onToggle={onFocusNumberToggle} onChange={onFocusNumberChange} />
            <NegativeNumbersToggle enabled={negativeNumbersEnabled} onToggle={onNegativeToggle} />
          </div>
        </CollapsibleContent>
      </Collapsible>
    </div>;
