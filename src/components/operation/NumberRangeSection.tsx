...
const NumberRangeSection = ({
  ...
  const renderRangeInputs = () => {
    if (range1 && range2 && setRange1Min && setRange1Max && setRange2Min && setRange2Max) {
      return <div className="flex flex-wrap md:flex-nowrap gap-8 max-w-[700px] mx-auto px-0">
          <div className="flex-1 min-w-[240px]">
            ...
            <div className="flex flex-row gap-4">
              <div className="flex flex-col">
                ...
                <input value={focusNumberEnabled ? focusNumber : range1.min} onChange={e => setRange1Min(e.target.value)} onFocus={selectAllOnFocus} disabled={focusNumberEnabled} className={`w-24 px-3 py-1 rounded-md border shadow-sm focus:ring-2 focus:ring-accent focus:border-accent font-mono text-lg text-left appearance-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${focusNumberEnabled ? 'bg-muted text-muted-foreground' : ''}`} type="number" inputMode="numeric" pattern="[0-9]*" />
              </div>
              <div className="flex flex-col">
                <label className="text-xs font-medium block mb-2">Max</label>
                <input value={focusNumberEnabled ? focusNumber : range1.max} onChange={e => setRange1Max(e.target.value)} onFocus={selectAllOnFocus} disabled={focusNumberEnabled} className={`w-24 px-3 py-1 rounded-md border shadow-sm focus:ring-2 focus:ring-accent focus:border-accent font-mono text-lg text-left appearance-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${focusNumberEnabled ? 'bg-muted text-muted-foreground' : ''}`} type="number" inputMode="numeric" pattern="[0-9]*" />
              </div>
            </div>
            {focusNumberEnabled && <p className="text-xs text-muted-foreground mt-2">
